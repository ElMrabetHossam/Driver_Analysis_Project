...
if __name__ == '__main__':
    ...
    wheel_speeds = Wheels.get('wheel_speeds', {}) if Wheels else {}
    slip_indicators = Wheels.get('slip', {}) if Wheels else {}
    wheel_dynamics_fig = create_wheel_dynamics_display(wheel_speeds, slip_indicators)

    radar_fig = create_radar_display(Targets)

    # Calculate dynamics metrics
    yaw_rate = Attitude.get('yaw_rate', 0) if Attitude else 0
    curvature_radius = data.get('curvature_radius', None)
    dynamics_fig = create_dynamics_metrics_gauge(yaw_rate, steering, speed_kmh, curvature_radius)

    # Create centrifugal force gauge
    centrifugal_fig = create_centrifugal_force_gauge(speed_kmh, curvature_radius)

    # Create ground truth path display
    gt_path_fig = create_ground_truth_path_display(GT_Path)
    ...
    return (speed_gauge, g_force, steer_gauge, map_fig, speed_graph, time_str, img_src, 
            slider_val, count_str, attitude_fig, wheel_dynamics_fig, radar_fig, 
            dynamics_fig, gt_path_fig)
